 ...
 function MetaballsTouch(gl, config, targetScreenSize){
     ...
     */
     this.handleTouchStart = function(event) {
        for (let touch of event.touches) {
            touchPoints[touch.identifier] = { x: touch.clientX, y: window.innerHeight - touch.clientY };
        }
    };
     ...
     this.handleTouchMove = function(event) {
        for (let touch of event.touches) {
            if (touchPoints[touch.identifier]) {
                touchPoints[touch.identifier].x = touch.clientX;
                touchPoints[touch.identifier].y = window.innerHeight - touch.clientY;
            }
        }
    };
    /**
     ...
     */
     this.handleTouchEnd = function(event) {
        touchPoints = touchPoints.filter((_, index) => !Array.from(event.touches).find(touch => touch.identifier === index));
    };
